...
function SettingsScreen({ theme, dark, onThemeChange, fontSize, setAndSaveFontSize, fontSizeMap }) {
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              // Em um app real, aqui implementaríamos a exportação de dados para um arquivo
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.card }]}>
      ...
      <View style={[styles.settingsOption, { borderBottomColor: theme.border }]}>
        <Text
          style={[styles.settingsOptionText, { color: theme.text, fontSize: fontSizeMap[fontSize] }]}
          accessibilityLabel="Selecionar tamanho da fonte"
        >
          Tamanho da Fonte
        </Text>
        <TouchableOpacity 
      ...
      <View style={{ marginTop: 20, alignItems: 'center' }}>
        <Text style={{ color: dark ? '#666' : '#999', fontSize: 12 }}>Versão 0.0.11</Text>
      </View>
    </View>
  );
}
