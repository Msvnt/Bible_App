...
function BibleScreen({ theme, dark, fontSize, fontSizeMap }) {
  ...
  const fetchVerses = async (book = selectedBook, chapter = selectedChapter) => {
    ...
    try {
      ...
      if (!parsed.includes(text)) {
        ...
        if (Platform.OS === 'android') {
          ToastAndroid.show('Versículo adicionado aos favoritos!', ToastAndroid.SHORT);
        } else {
          Alert.alert('Favorito', 'Versículo adicionado aos favoritos!');
        }
      } else {
        if (Platform.OS === 'android') {
          ToastAndroid.show('Versículo já está nos favoritos', ToastAndroid.SHORT);
        } else {
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.card }]}>
      <Text style={[styles.subtitle, { color: theme.text, fontSize: fontSizeMap[fontSize] }]}>
        📖 Leitura da Bíblia
      </Text>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        ...
        >
          <Text style={{ color: '#fff', fontWeight: '600', fontSize: fontSizeMap[fontSize] }}>
      ...
      <SelectionModal 
        ...
        items={[...Array(BOOKS_WITH_CHAPTERS[selectedBook] || 0)].map((_, i) => `Capítulo ${i + 1}`)}
      ...
      {verses.length > 0 ? (
          ...
          <ScrollView ref={scrollRef} contentContainerStyle={{ paddingBottom: 20 }}>
            {verses.map((v) => (
              <TouchableOpacity
                ...
                <Text style={{ color: theme.text, marginBottom: 6, fontSize: fontSizeMap[fontSize] }}>
                  <Text style={{ fontWeight: 'bold' }}>{v.verse}.</Text> {v.text}
                </Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        ...
        <Text style={{ color: theme.text, fontSize: fontSizeMap[fontSize], marginTop: 20 }}>
          {selectedBook && selectedChapter
